...
export function generateLevelWithAI(meta) {
  ...
  objects.push(createObject("block", 0, groundY - 30, { width: 360, height: 30 }));
  ...
  objects.push(createObject("block", x, groundY - 30, { width: 200, height: 30 }));

  if (rand() < profile.orbRate) {
    objects.push(createObject("orb_yellow", x - 120, groundY - 120));
  }
...
function applySegment(objects, x, groundY, type, profile, rand) {
  switch (type) {
    case "gap": {
      ...
      x += gap;
      objects.push(createObject("block", x, groundY - 30, { width: 120 + rand() * 180, height: 30 }));
      x += 120;
      break;
    }
    case "spikes": {
      ...
      for (let i = 0; i < count; i++) {
        objects.push(createObject("spike", x + i * 35, groundY - 58));
      }
      objects.push(createObject("block", x - 30, groundY - 30, { width: count * 35 + 60, height: 30 }));
      x += count * 35 + 30;
      break;
    }
    case "stairs": {
      for (let s = 0; s < 4; s++) {
        objects.push(createObject("block", x + s * 40, groundY - 30 - s * 30, { width: 50, height: 30 }));
      }
      x += 200;
      break;
    }
    default:
      objects.push(createObject("block", x, groundY - 30, { width: 90 + rand() * 120, height: 30 }));
      x += 90;
  }
  return x;
}
